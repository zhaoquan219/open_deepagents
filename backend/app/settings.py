...
class Settings(BaseSettings):
    ...
    def prepare_paths(self) -> None:
        ...
        if raw_path == ":memory:":
            return
        path = Path(raw_path)
        (path if path.is_absolute() else BACKEND_ROOT / path).parent.mkdir(
            parents=True,
            exist_ok=True,
        )
    ...
    def runtime_driver_mode(self) -> str:
        mode = (self.deepagents_runtime_driver or "memory").strip().lower()
        if mode not in {"memory", "sqlite", "postgres", "factory"}:
            raise ValueError(
                "DEEPAGENTS_RUNTIME_DRIVER must be memory, sqlite, postgres, or factory"
            )
        return mode
    ...
    def sandbox_settings(self) -> dict[str, Any]:
        default_root_dir = str((BACKEND_ROOT / "agents").resolve())
    ...
    def runtime_database_url_for_builtin(self, mode: str) -> str:
        if mode not in {"sqlite", "postgres"}:
            raise RuntimeError(
                "Only sqlite/postgres official runtime modes use DEEPAGENTS_RUNTIME_DATABASE_URL."
            )
        if not self.deepagents_runtime_database_url:
            raise RuntimeError(
                f"DEEPAGENTS_RUNTIME_DRIVER={mode} requires DEEPAGENTS_RUNTIME_DATABASE_URL "
        ...
        if _driver_family(self.deepagents_runtime_database_url) != expected:
            raise RuntimeError(
                f"DEEPAGENTS_RUNTIME_DRIVER={mode} requires a {expected} "
                "DEEPAGENTS_RUNTIME_DATABASE_URL."
            )
        if _database_target(self.deepagents_runtime_database_url) == _database_target(
            self.database_url
        ):
            raise RuntimeError(
                "DEEPAGENTS_RUNTIME_DATABASE_URL must be different from DATABASE_URL; "
                "product DB schema is exactly users, sessions, runs, and events."
            )
        return self.deepagents_runtime_database_url
...
def _create_runtime_components(*, mode: str, database_url: str) -> dict[str, Any]:
    specs = {
        "sqlite": (
            ...
            "SqliteStore",
            database_url.removeprefix("sqlite+pysqlite:///").removeprefix("sqlite:///"),
        ),
        "postgres": (
    ...
    try:
        ...
        raise RuntimeError(
            f"DEEPAGENTS_RUNTIME_DRIVER={mode} requires official LangGraph "
            f"checkpoint/store packages (langgraph-checkpoint-{mode}) or "
            "DEEPAGENTS_RUNTIME_FACTORY."
        ) from exc
    return {
        "checkpointer": getattr(checkpoint, checkpoint_class).from_conn_string(connection),
        "store": getattr(store, store_class).from_conn_string(connection),
        "cache": None,
    }
