...
## 会话级自定义状态（Session-scoped custom state）

后端现在内置了一套轻量的会话级状态存储，适合给异步任务、外部预处理器和
run-input hook 做协作。

适用场景包括：

- 异步任务把进度或结果回写到当前 session
- 保存小型结果载荷，例如 `index_id`
- 让 run-input hook 在执行前读取 ready 状态并注入提示词
- 注入完成后把状态标记为已消费，避免重复注入
...
- `status`，例如 `pending`、`ready`、`consumed`、`failed`
- `consume_policy`，当前支持 `once` 和 `keep`
- `value`，JSON 载荷
- `version`
- 可选的 `expires_at`
- 消费元数据，例如 `last_consumed_at`、`last_consumed_run_id`
...
仓库已内置一个 run-input hook 示例：

- `backend/extensions/runtime_hooks/session_state_hooks.py`

这个 hook 会：

1. 读取 `log_ingestion` namespace 下的 `ready` 状态
2. 把状态压缩成简洁的 prompt block 注入当前用户消息
3. 通过 session-state helper 把消费过的状态安全标记为 consumed

默认聚合器已经把这个 hook 和附件 hook 一起接进去了：

```dotenv
DEEPAGENTS_RUN_INPUT_HOOK_SPECS=extensions/runtime_hooks/__init__.py:RUN_INPUT_HOOKS
```

如果你要改成自己的 namespace 或格式策略，建议复制这个模块并调整：
