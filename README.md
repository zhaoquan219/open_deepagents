...
- CRUD for sessions, messages, uploads, and session-scoped custom state
...
## Session-scoped custom state

The backend now includes a small session-scoped state store for long-running jobs,
external preprocessors, and run-time prompt injection.

Use it when you need to:

- write async job progress back into the current session
- store a small result payload such as an `index_id`
- let a run-input hook consume ready state and inject it into the prompt
- mark a state item as consumed so it is not injected twice

### State object shape

Each state item is uniquely identified by:
...
- `status` (for example `pending`, `ready`, `consumed`, `failed`)
- `consume_policy` (`once` or `keep`)
- `value` (JSON payload)
- `version`
- optional `expires_at`
- consume metadata such as `last_consumed_at` and `last_consumed_run_id`

### State API

Authenticated admin clients can use these endpoints:

- `GET /api/sessions/{session_id}/state`
- `GET /api/sessions/{session_id}/state/{namespace}/{key}`
- `PUT /api/sessions/{session_id}/state/{namespace}/{key}`
- `PATCH /api/sessions/{session_id}/state/{namespace}/{key}`
- `DELETE /api/sessions/{session_id}/state/{namespace}/{key}`
- `POST /api/sessions/{session_id}/state/{namespace}/{key}/consume`

### Example: async worker writes state

Create or replace a ready state item:

```bash
curl -X PUT "http://127.0.0.1:8000/api/sessions/sess_123/state/log_ingestion/upload:upl_123" \
...
Patch progress while a job is still running:

```bash
curl -X PATCH "http://127.0.0.1:8000/api/sessions/sess_123/state/log_ingestion/upload:upl_123" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "status": "pending",
...
Consume one state item explicitly:

```bash
curl -X POST "http://127.0.0.1:8000/api/sessions/sess_123/state/log_ingestion/upload:upl_123/consume" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"run_id": "run_123"}'
```
...
`backend/extensions/runtime_hooks/session_state_hooks.py` contains a bundled run-input
hook example.

The hook:

1. reads `ready` state from the `log_ingestion` namespace
2. injects a compact prompt block into the current user message
3. marks consumed items through the session-state helper

The bundled runtime hook aggregator already includes this example together with the
attachment hook:
...
If you want a custom namespace or formatting policy, copy that module and adjust:

- `SESSION_STATE_NAMESPACE`
- the prompt rendering logic
- the consume limit and compaction rules
