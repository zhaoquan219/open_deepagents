...
describe('processLog', () => {
  ...
  it('keeps subagent and error events visible while internal logs are hidden', () => {
    ...
    expect(subagent).toMatchObject({ kind: 'subagent', title: 'code-reviewer' })
  ...
  it('folds adjacent intermediate events into one chronological process group', () => {
    ...
    expect(groups[0]).toMatchObject({
      kind: 'process',
      title: '中间过程',
      items: expect.arrayContaining([expect.objectContaining({ summary: 'A' })]),
    })
    expect(groups[0].items).toHaveLength(3)
  })

  it('drops empty thinking blocks before rendering process logs', () => {
    expect(thinkingEntriesFromContent('<think>  </think>\n\n完成', '2026-05-05T00:00:00Z')).toEqual([])
  })

  it('keeps adjacent mixed tool history under one intermediate process group after refresh', () => {
    const groups = groupProcessLogs([
      { id: '1', kind: 'tool', title: 'read_file', summary: 'input', timestamp: '2026-05-05T00:00:00Z' },
      { id: '2', kind: 'tool', title: 'read_file', summary: 'output', timestamp: '2026-05-05T00:00:01Z' },
      { id: '3', kind: 'tool', title: 'grep', summary: 'matches', timestamp: '2026-05-05T00:00:02Z' },
    ])

    expect(groups).toHaveLength(1)
    expect(groups[0]).toMatchObject({ kind: 'process', title: '中间过程' })
    expect(groups[0].items.map((item) => item.title)).toEqual(['read_file', 'read_file', 'grep'])
  })
})
